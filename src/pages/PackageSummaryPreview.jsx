...
const TourPackageSummary = () => {
  ...
  <div className="p-6 max-w-4xl mx-auto">
    ...
    <Skeleton variant="rectangular" width="100%" height={150} className="my-4" />
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-8 mb-8">
      ...
      <div className="border-b pb-6">
        ...
        <div className="flex items-center gap-4 flex-wrap text-gray-600 text-sm">
          <Chip label={tour.basicInfo?.tour_type} color="primary" size="small" />
      ...
      <Card title="Pricing">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-2xl font-semibold text-blue-600">
              {tour.pricing?.currency} {Number(tour.pricing?.pricePerPerson).toLocaleString()}
            </p>
          </div>
          <div>
            ...
            <ul>
              {tour.pricing?.availability.map((a, idx) => (
                <li key={idx} className="flex justify-between text-sm">
                  ...
                  <span className={a.is_available ? 'text-green-600' : 'text-red-600'}>
        ...
        {tour.pricing?.discount && (
          <div className="mt-4 text-sm text-gray-600">
            ...
            <p>
              Valid: {new Date(tour.pricing.discount.startDate).toLocaleDateString()} –{" "}
              ...
              <div className="space-y-4">
                ...
                <div className="text-sm text-gray-700 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    {new Date(tour.itinerary.startTime).toLocaleString()} – {new Date(tour.itinerary.endTime).toLocaleString()}
                  </div>
                  <div className="flex items-center gap-2">
                    <LocationOnIcon className="text-blue-500" fontSize="small" />
      ...
      <Card title="Media & Assets">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {tour.media.tourImages.map((img, i) => (
                <a key={i} href={img.url} target="_blank" className="text-blue-600 underline text-xs">
                  Image {i + 1}
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {tour.media.tourVideos.map((vid, i) => (
                <a key={i} href={vid.url} target="_blank" className="text-blue-600 underline text-xs">
                  Video {i + 1}
                </a>
              ))}
            </div>
        ...
        {tour.media?.additionalFiles?.length > 0 && (
          <div>
            <p className="font-medium text-sm text-gray-500 mt-4">Other Files</p>
            <div className="flex flex-wrap gap-2">
              {tour.media.additionalFiles.map((file, i) => (
                <a key={i} href={file.url} target="_blank" className="text-blue-600 underline text-xs">
                  File {i + 1}
                </a>
              ))}
            </div>
